# main_qt_v4_save_view.py
# 增加了“保存当前视图”功能，可将屏幕上显示的数据精确导出为CSV文件
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Interactive Sensor Monitor with Export")
    ...
    def redraw_plots(self):
        ...
        s1_slice, s2_slice, diff_slice = self.full_history_s1[start_idx:end_idx], self.full_history_s2[start_idx:end_idx], self.full_history_diff[start_idx:end_idx]
    ...
    # ===================================================================
    # =====          vvv   新功能：保存文件的槽函数 vvv               =====
    # ===================================================================
    @QtCore.Slot()
    def save_current_view(self):
        """响应“保存”按钮点击，将当前视图的数据写入CSV文件。"""
        # 1. 弹出“另存为”对话框
        # 建议一个默认文件名，包含时间戳以保证唯一性
        ...
        # 2. 获取当前视图的数据切片 (逻辑与redraw_plots完全一致)
        window_size = self.window_size_spinbox.value()
        start_idx = self.scroll_slider.value()
        end_idx = start_idx + window_size
        ...
        try:
            ...
            # 将数据包列表展开为数据点列表
            s1_points = np.concatenate([np.column_stack(p) for p in s1_slice])
            s2_points = np.concatenate([np.column_stack(p) for p in s2_slice])

            with open(file_path, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(['timestamp', 'sensor_id', 'pressure'])  # 写入表头

                for row in s1_points:
                    writer.writerow([row[0], 1, row[1]])  # 写入传感器1数据

                for row in s2_points:
                    writer.writerow([row[0], 2, row[1]])  # 写入传感器2数据
